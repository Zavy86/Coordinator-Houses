 ...
 */
 ...
 if(ACTION=="room_view"){
  ...
  // build modal
  $modal=new strModal(api_text("houses_view-rooms-modal-title-view",$house_obj->name),null,"houses_view-rooms");
  $modal->setBody($dl->render(1));
  // add modal to house
  $app->addModal($modal);
  // modal scripts
  $app->addScript("$(function(){\$('#modal_houses_view-rooms').modal({show:true});});");
 }
 ...
 if(in_array(ACTION,["room_add","room_edit"]) && api_checkAuthorization("houses-houses_manage")){
  ...
  $form->addControl("button",api_text("form-fc-remove"),api_url(["scr"=>"controller","act"=>"remove","obj"=>"cHousesHouseRoom","idHouse"=>$house_obj->id,"idRoom"=>$selected_room_obj->id,"return"=>["scr"=>"houses_view","idHouse"=>$house_obj->id]]),"btn-danger",api_text("houses_view-rooms-modal-fc-remove-confirm"));
  // build modal
  $modal=new strModal(api_text("houses_view-rooms-modal-title-".($selected_room_obj->id?"edit":"add"),$house_obj->name),null,"houses_view-rooms");
  $modal->setBody($form->render(1));
  // add modal to house
  $app->addModal($modal);
  // modal scripts
  $app->addScript("$(function(){\$('#modal_houses_view-rooms').modal({show:true,backdrop:'static',keyboard:false});});");
 }
?>